...
async def start_command(message: types.Message):
    await message.answer("Привет, я бот погоды. Ты называешь мне город, а я тебе говорю в нём погоду!", reply_markup=button.CITY)


@dp.callback_query_handler(text='city')
async def city(callback: types.CallbackQuery):
    await callback.message.answer('Вводи название города)')
...
async def what(callback: types.CallbackQuery):
    await callback.message.answer('Вводи название нового города)')
    await callback.answer()
